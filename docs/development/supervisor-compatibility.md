...
Verify such a record with `pnpm openshell:runtime-diagnostic:check <record.json> --source-commit <exact-commit> --candidate-image sha256:<runtime-id> --supervisor-candidate-image sha256:<supervisor-id>`. Both images must be supplied independently; the verifier recomputes the realized gateway digest from the checked template and exact selected supervisor. A missing binding, changed template, substituted image, patch drift or downgrade to an older record version fails. The v5 record remains non-certifying. The separate [strict local acceptance contract](local-runtime-acceptance.md#strict-runtime-acceptance) can bind it to both published images, but the current governed worker still accepts only the original v3 diagnostic through its version 1 acceptance profile. No successful v5 runtime record is incorporated here.

On 2026-09-16, [publication run 35150404485, attempt 1](https://github.com/asabla/dataground/actions/runs/35150404485/attempts/1) completed successfully from reviewed source `b75720fec290176c3451488a60079d2be6cc0563`. The online verifier independently checked its signature, exact completed invocation, and pulled ARM64 image `ghcr.io/asabla/dataground-supervisor-candidate@sha256:57cc182a7c56cf1fbd64af8b09b2196eea727783eb9ec0c2292542e3be0f755c`. This records publication verification only; a fresh live diagnostic against that published image, signed runtime acceptance, and worker activation remain separate gates.
