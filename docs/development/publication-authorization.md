...
The publication authorizer supplies current policy decisions for the explicitly configured [governed publication API and worker](governed-publication-queue.md#publish-through-the-authenticated-api). It uses exact-revision policy installation, entity refresh, and irreversible withdrawal. The strict invocation worker accepts version 3 approval policies or version 5 publication policies. The operator publication commands retain version 3; the authorized consumer requires version 5. This is a loopback development composition and does not establish release acceptance.
