...
Admission and finalization are internal and unwired. The logical object ports, finalizer, PostgreSQL catalog, and narrow S3 REST adapter exist, but there is no certified backend, bucket or workload-identity configuration, public API, authorization decision, publication/invocation reconciler effect, or live sandbox call through this path. The adapter requires an explicit operator-supplied transport, blocks redirects, permits plain HTTP only for an explicitly enabled loopback endpoint, and exposes only safe missing/conflict outcomes. The eventual deployment transport must add workload authentication; that behavior is not implemented or certified here. Persisted provenance is evidence, not authorization or Rosetta certification. Missing plans, cross-domain or revision-mismatched bundles, malformed identities, digest mismatches, absent or unavailable objects, unavailable placement, missing or unsafe policy workspace, and cancelled provider calls fail closed. Rosetta certification and production object storage remain separate gates.
