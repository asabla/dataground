...
An enforcement-bundle identifier is not a filesystem path. Internal execution admission retrieves the bundle through an isolation-scoped source, requires the bundle to match the plan's domain, revision, identifier, and digest, verifies the content hash before placement, and passes an owned byte slice to `ExecutionProvider.Create`. Provider profiles are names, not embedded provider configuration or credentials. The plan deliberately contains no gateway endpoint, sandbox name, native runtime identifier, local policy path, or secret.
...
Admission is internal and unwired. There is no production enforcement-bundle source, public API, authorization decision, publication/invocation reconciler effect, or live sandbox call through this path. Missing plans, cross-domain or revision-mismatched bundles, malformed identities, digest mismatches, unavailable placement, missing or unsafe policy workspace, and cancelled provider calls fail closed. Rosetta certification and production object storage remain separate gates.
