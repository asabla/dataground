...
The OpenShell provider no longer accepts a caller-controlled policy path. It labels the sandbox with an opaque fingerprint of the immutable image, policy digest, normalized provider profiles, isolation domain, and operation; observation recovers a lost acknowledgement only when that fingerprint matches, while a conflicting retry fails closed. After confirming the deterministic sandbox does not already exist, the provider writes the verified bytes to a mode-`0600` temporary file, passes that private path to the synchronous OpenShell CLI call, and removes the file on success and recoverable failure paths. A process crash can leave that file behind; production wiring therefore requires an operator-owned private runtime directory and startup cleanup policy.

Admission is internal and unwired. There is no production enforcement-bundle source, public API, authorization decision, publication/invocation reconciler effect, or live sandbox call through this path. Missing plans, cross-domain or revision-mismatched bundles, malformed identities, digest mismatches, unavailable placement, and cancelled provider calls fail closed. Rosetta certification and production object storage remain separate gates.
