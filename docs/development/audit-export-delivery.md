...
The internal `dataground-audit-export-delivery` command records the durable lifecycle around an audit transport. DataGround prepares one isolation-scoped delivery for an exact [recipient-encrypted package](audit-export-encryption.md) only while its recipient profile and encryption key are the latest active, identity-proven [trust generation](audit-export-recipient-trust.md). New version 5 deliveries require a completed [immutable S3 transport](audit-export-transport.md) under either the anonymous loopback or pinned mTLS contract before the command can verify and record a recipient-signed acknowledgement against the same generation.
...
Exact preparation, transport, and acknowledgement replays are read-only. Reusing a delivery identifier, operation correlation, envelope or package identity, destination binding, transport contract, attribution, receipt, recipient trust profile, either recipient key, or trust generation with different values fails closed. The database permits only operation-bound transitions under the exact prepared generation, rejects deletion, and keeps delivery, transport, operation, trust-event, and trust-key evidence append-only. Concurrent trust rotation, revocation, proof-revocation intake, preparation, transport reservation, and acknowledgement serialize around the same domain and recipient. Schema 29 preserves all version 5 loopback evidence while allowing the exact mTLS transport contract and refusing downgrade when authenticated transport evidence exists.
...
An acknowledged version 5 row proves that DataGround prepared the exact recipient-encrypted package under the then-current authorized X25519 key, verified the same bytes at the destination-bound immutable object under the recorded transport contract, and accepted a receipt signed by a key from the same trust generation. For the mTLS contract, the destination additionally binds the exact client certificate and pinned server trust bytes used before the operation was reserved. It also proves that the generation's proof remained unexpired and outside the effective recorded revocation set at preparation, transport reservation, and acknowledgement. It does not prove workload-identity issuance quality, certificate revocation, server-side authorization, recipient plaintext retention, external authority process quality, or plaintext source erasure. Production transport certification, proofing- and revocation-authority governance, external evidence and notice acquisition, recipient access, retention, legal hold, secure deletion, and independent cryptographic-policy review remain deployment boundaries. Historical version 4 acknowledgements retain their narrower pre-transport claim.
