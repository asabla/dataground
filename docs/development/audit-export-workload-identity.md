...
Grant activation, local revocation, external issuer-revocation intake, and [revocation-authority changes](audit-export-revocation-authorities.md) audit credential identifiers and digests without storing certificate bytes, private keys, signed evidence bytes, or authority public keys. External certificate and grant issuance, private-key delivery, revocation-notice acquisition, authority selection and profile distribution, server authorization and bucket policy, DNS governance, production topology, and production release certification remain deployment and release boundaries. See [audit export workload identity revocation](audit-export-workload-identity-revocation.md) for the external notice contract and enforcement boundary.
