...
Grant activation, local revocation, external issuer-revocation intake, [revocation acquisition](audit-export-revocation-acquisition.md), and [revocation-authority changes](audit-export-revocation-authorities.md) audit credential identifiers and digests without storing certificate bytes, private keys, signed evidence bytes, authority public keys, bearer credentials, or response bodies. External certificate and grant issuance, private-key delivery, acquisition-credential issuance and remote revocation, authority and source selection, server authorization and bucket policy, DNS governance, production topology, and production release certification remain deployment and release boundaries. See [audit export workload identity revocation](audit-export-workload-identity-revocation.md) for the external notice contract and enforcement boundary.
