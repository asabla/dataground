...
New activations require the encryption-capable version 2 trust profile and version 3 authorization evidence. Delivery preparation and acknowledgement each resolve trust within the same PostgreSQL transaction and advisory scope as their durable transition. Preparation requires the exact profile digest and X25519 key to belong to the latest identity-proven active generation. Acknowledgement additionally requires its Ed25519 key and pinned generation to match that exact preparation. Both transitions require the proof to remain unexpired according to the PostgreSQL clock and reject a matching effective [external proof revocation](audit-export-recipient-proof-revocation.md). Concurrent activation, rotation, revocation intake, preparation, or acknowledgement therefore completes before or after a consequential transition, never between authorization and its durable write. Later expiry or revocation does not rewrite completed evidence, and exact replay remains read-only.

Historical signing-only version 1 profiles and version 2 authorization events remain readable for pending version 3 acknowledgement and exact replay, but no new activation may use them. This boundary proves only that a configured external authority signed the exact identity and key-custody binding DataGround accepted, and that the proof remained within its declared validity and outside the recorded effective revocation set when preparation or acknowledgement committed. Selecting and governing proofing and revocation authorities, defining and retaining underlying evidence, acquiring and delivering external notices, independent clock policy, transport execution, recipient decryption and access, retention, legal hold, and deletion remain deployment responsibilities.
