...
For a new version 3 delivery, acknowledgement and trust resolution share one PostgreSQL transaction and advisory scope. The transition requires the receipt's exact profile digest and signing key to belong to the latest identity-proven active generation, requires that generation's proof to remain unexpired according to the PostgreSQL clock, and rejects a matching effective [external proof revocation](audit-export-recipient-proof-revocation.md). Activation applies the same external-revocation check. Concurrent activation, rotation, revocation intake, or acknowledgement therefore completes before or after the consequential transition, never between authorization and the durable write. Later expiry or revocation does not rewrite a completed acknowledgement, and exact acknowledgement replay remains read-only. Completed version 2 deliveries retain their version 1 receipt evidence without gaining an authorization or identity-proofing claim; only pending deliveries upgrade to version 3.

This boundary proves only that a configured external authority signed the exact identity and key-custody binding DataGround accepted, and that the proof remained within its declared validity and outside the recorded effective revocation set when a delivery acknowledgement committed. Selecting and governing proofing and revocation authorities, defining and retaining underlying evidence, acquiring and delivering external notices, independent clock policy, transport execution, encryption, recipient access, retention, legal hold, and deletion remain deployment responsibilities.
