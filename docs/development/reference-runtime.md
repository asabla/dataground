...
Migration 47 adds the retained withdrawal marker and the audited action. Withdrawal locks the exact alias and commits its state, safe audit, outbox event, and idempotency receipt together; invocation admission locks the same route. Retirement ignores withdrawn aliases, while assigning a route still locks and requires a published revision. Upgrade the database before starting these API and worker binaries. Downgrade refuses retained withdrawal state or historical withdrawal audit, even after a name is reassigned. The Workbench understands later versions when recreating an absent route; the dedicated withdrawal control is not yet exposed there.
