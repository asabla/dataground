...
Clients can discover all active routes with `GET /v1/isolation-domains/{isolationDomainId}/agent-services/{serviceId}/aliases`, authorized by `listServiceAliases` on the exact service. Pages default to 50 aliases and accept limits from 1 to 100, ordered by name in ascending ASCII order. An opaque versioned cursor binds the domain, service, and last name. Withdrawn aliases are excluded; the cursor continues after its name even if that route has since been withdrawn. Each page reads current state, so concurrent assignment or recreation before the cursor requires refreshing from the first page. This is route discovery, not a historical or frozen snapshot. The Workbench's existing service-open flow still reads `stable`; the listing API enables custom-route discovery clients.
