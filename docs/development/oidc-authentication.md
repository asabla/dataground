...
The repository does not execute the external authorization or client-credentials flow, register an OAuth client, acquire provider credentials, or mint a token. The deployment-owned conformance runner and reviewer remain responsible for those external effects and their sensitive inputs. The resulting envelope is not yet incorporated into API startup or the signed loopback release certification, so it is reviewable evidence rather than production activation.
...
This is not production API activation. A deployment still needs an authorization server that issues DPoP-bound access tokens and must run the external conformance flow represented by the signed issuance-evidence boundary. The API does not yet incorporate that evidence. Reviewed TLS ingress that routes the configured origin without rewriting its canonical path, a reviewed decision to enable and size nonce enforcement, upstream provider credential acquisition and remote revocation, complete production release certification beyond the signed loopback slice, and reviewed non-loopback deployment activation also remain required. The executable's OIDC profile therefore remains loopback-only.
