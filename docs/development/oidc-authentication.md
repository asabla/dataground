...
Local revocation uses the next generation with `operation` set to `revoke`, includes a `revokedAt` no more than five minutes from the command clock, and omits `activatedAt`, `expiresAt`, and `bearerTokenFile`. It retains the isolation domain, actor, reason, and correlation fields and installs a timestamped durable tombstone instead of deleting the prior state; a later locally acquired credential may be activated only at the following generation. A publication path remains permanently scoped to its first isolation domain, provider, and endpoint, while a new registry digest may be adopted only through the next generation. This boundary audits DataGround's local credential use but does not acquire a credential from the provider or revoke it at the provider. The internal [operator audit export](operator-audit-export.md) can extract the safe provider-binding, generation, endpoint, attribution, and reason-digest fields through a frozen receipt-bound page without credential bytes or credential digests. Exact signed pages can be [encrypted](audit-export-encryption.md) to an identity-proven X25519 recipient, verified at a bound immutable S3 object under that same active [recipient trust generation](audit-export-recipient-trust.md) and an active signed [transport workload identity](audit-export-workload-identity.md), and acknowledged through an exact [recipient-signed receipt](audit-export-delivery-receipts.md) without a matching effective [proof revocation](audit-export-recipient-proof-revocation.md), but provider-side issuance, remote revocation, custody before installation, external transport-identity and recipient-proof issuance, acquisition-credential issuance and remote revocation, complete external monitoring, production certification, access, and retention remain deployment responsibilities.
...
## Provider DPoP issuance evidence

DataGround remains a protected resource and does not implement an authorization server. The provider-neutral [`dataground-provider-dpop-issuance`](provider-dpop-issuance.md) boundary verifies a short-lived externally signed certification for one exact provider, registry, issuer, token endpoint, audience, grant, algorithm, nonce-behavior, and canonical conformance report. The closed report must prove DPoP token issuance and resource use while also proving missing-proof, wrong-key, and replay denials, and it contains no token, proof, code, provider response, subject, client secret, or private key. Verification fails closed on provider drift, report mutation or loss, signer substitution, stale observation, or expiry.

The repository does not execute the external authorization or client-credentials flow, register an OAuth client, acquire provider credentials, or mint a token. The deployment-owned conformance runner and reviewer remain responsible for those external effects and their sensitive inputs. The signed loopback release certificate now incorporates the exact envelope and its separate trust profile, cross-binds its provider, registry, issuer, and audience to the OIDC configuration, and carries its shorter expiry into API readiness. This authenticates one reviewed observation; it does not operate or continuously monitor the authorization server.
...
The provider registry, endpoint-scoped local credential state, authenticated import, published keyset generation, reloadable verifier, signed configuration, and incorporated provider DPoP issuance evidence now share one exact provider identity and registry digest. Upstream provider credential acquisition and remote revocation, group-to-membership administration, HTTPS ingress deployment, external conformance execution and monitoring, production release certification beyond the signed loopback OIDC slice, workload identity, and production conformance remain unresolved. Resource-server nonce enforcement and its capacity evidence are implemented, but nonce selection and thresholds remain explicit deployment choices inside the signed loopback configuration. The opt-in OIDC profile and default static development identity both require explicit loopback listeners and must not bind publicly.


## Signed loopback release certification

`dataground-release-certification` can bind one clean build and released Go runtime to the exact owner-only OIDC security configuration, accepted admission-and-nonce capacity record, Cedar API policy, short-lived externally signed provider DPoP issuance envelope and its separate trust profile, reviewer attribution, bounded validity window, and deployment-owned Ed25519 release trust profile. It prepares the exact domain-separated message for an external signer, verifies the detached signature without loading private key material, and installs a new immutable envelope through durable same-directory linking. Exact replay re-syncs the directory; a different existing envelope conflicts.

The statement cross-checks the capacity record's accepted status, source revision, Go runtime, deployment profile, and explicit nonce selection, then requires the OIDC configuration to name that exact capacity file and digest, Cedar policy path, provider identity, and provider-registry digest. It verifies the provider envelope from the same signed artifact bytes, authenticates it against the exact signed provider trust profile, revalidates the canonical conformance report, and requires its provider, registry, issuer, and audience to match the OIDC configuration. The signed profile remains limited to loopback OIDC and is not external conformance execution, ingress evidence, infrastructure certification, or non-loopback activation. See [signed release certification](release-certification.md) for the closed contracts and operator procedure.

OIDC startup requires the installed envelope through `DATAGROUND_RELEASE_CERTIFICATION_FILE` and its public trust profile through `DATAGROUND_RELEASE_CERTIFICATION_TRUST_FILE`. These bootstrap paths deliberately remain outside the signed OIDC configuration to avoid a circular envelope digest. Verification instead requires the signed statement to name and digest the exact configured `DATAGROUND_API_SECURITY_CONFIG_FILE`, then cross-binds its capacity evidence and Cedar policy. The API opens no database connection when certification is missing, invalid, expired, for another clean build, or for another configuration path or content. Every non-liveness request remains unavailable once either the release certificate or its incorporated provider issuance evidence expires; the earlier expiry governs readiness, and clock rollback cannot restore it after expiry is observed. Renewal, evidence replacement, or either trust rotation requires a new verified release envelope and process restart.
...
This is not production API activation. A deployment still needs an authorization server that issues DPoP-bound access tokens and must run the external conformance flow represented by the incorporated signed issuance-evidence boundary. Reviewed TLS ingress that routes the configured origin without rewriting its canonical path, a reviewed decision to enable and size nonce enforcement, upstream provider credential acquisition and remote revocation, ongoing provider monitoring, complete production release certification beyond the signed loopback slice, and reviewed non-loopback deployment activation also remain required. The executable's OIDC profile therefore remains loopback-only.
