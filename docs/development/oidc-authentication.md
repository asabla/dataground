...
OIDC discovery, deployment keyset publication and refresh scheduling, provider revocation policy, group-to-membership administration, HTTPS ingress deployment, provider-side DPoP issuance, optional nonce policy, API startup configuration, rate limiting, workload identity, and production conformance remain unimplemented. Until those boundaries exist, the executable API continues to require its loopback-only static development identity and must not bind publicly.
