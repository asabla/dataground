...
DataGround has an internal provider-neutral boundary for turning a verified OIDC access token into a platform principal, plus a concrete verifier for one immutable JWT access-token profile. The API package also has opt-in DPoP request binding and pre-authentication rate-limit boundaries for composing trusted HTTP request metadata and deployment-owned admission. None of these boundaries is composed into the API command, and they do not make the reference server a production endpoint.
