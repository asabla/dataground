...
The internal [authorization audit export](authorization-audit-export.md) command can extract frozen, isolation-scoped pages from the API and invocation decision tables only after recording the exact request and content digests in an append-only receipt. Exact signed pages can be prepared for a deployment-owned destination and acknowledged only through an exact [recipient-signed receipt](audit-export-delivery-receipts.md) under the latest active [recipient trust generation](audit-export-recipient-trust.md) in the separate [delivery lifecycle](audit-export-delivery.md). Production OIDC verification, immutable production policy distribution, entity snapshots and hierarchy, export transport, encryption, recipient identity proofing and external trust revocation, retention and access policy, and policy administration remain deployment boundaries. The separate invocation-effect authorizer still governs admission, runtime, and cancellation immediately before consequential provider effects; entry authorization does not replace that second decision.
