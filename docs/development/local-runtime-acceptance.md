...
The strict receipt adds the published supervisor image, its exact local image ID, the gateway configuration digest, and the enforcement-policy digest. These values come from the verified evidence. This command does not inspect a running deployment. The current governed worker still accepts only the version 1 local profile; version 2 worker activation and a verified running topology remain separate work. Neither acceptance version supplies production certification.
