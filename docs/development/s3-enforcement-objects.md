...
A second two-phase runner joins the real object backend with the PostgreSQL enforcement catalog. Its preparation process verifies a fresh scope, writes the immutable object, closes its database pool immediately after the successful write, and requires finalization to fail safely before any relational binding or audit. CI then restarts the PostgreSQL server and starts a fresh recovery process. Recovery requires the object to be present and the catalog record to be absent, adopts the retained object through the production PostgreSQL store, proves an exact replay performs no further write, and requires exactly one catalog record and binding audit. The report contains only the schema, run identifier, phase, stable case names, and pass/fail states.
...
The remaining production gate is intentionally broad. The live development evidence now includes one disposable PostgreSQL server restart between separate finalizer processes, but not a PostgreSQL cluster, object-store restart, network partition, concurrent recovery workers, or measured recovery objective. Production still requires an operator-owned workload-authenticated transport and bucket policy; TLS, encryption/KMS, versioning, retention, multipart and presigned-operation behavior; multi-gateway concurrency and network partitions; replication, backup/restore, upgrades and rollback; Kubernetes operations and observability; a replacement backend; and the platform-object and lakehouse cases from ADR-035. None of those claims follow from this development subset.
