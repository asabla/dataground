...
Removing the signing-message, statement, or signature inputs follows the deployment's approved sensitive-file procedure. The installed envelope and public trust profile are evidence records and should be retained according to the release-evidence policy. A valid envelope proves only the fixed loopback OIDC profile named by its contract; production certification still requires the remaining release-candidate evidence and a separately reviewed activation boundary.
