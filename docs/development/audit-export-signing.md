...
The signature authenticates the exact export bytes and the reviewed trust profile. It does not prove to a verifier without database access that the local export receipt exists, and it does not encrypt the embedded records. Signing authorization, private-key lifecycle and signing-system audit remain deployment-owned. A separate [delivery acknowledgement command](audit-export-delivery.md) can bind an exact verified envelope to a deployment-owned destination and require both a [recipient-signed receipt](audit-export-delivery-receipts.md) and the latest active identity-proven [recipient trust generation](audit-export-recipient-trust.md) without a matching effective [proof revocation](audit-export-recipient-proof-revocation.md) before acknowledgement. Upload and transport execution, encryption, proofing- and revocation-authority governance, external notice acquisition, access policy, retention, legal hold, and deletion remain separate boundaries.
