...
The signature authenticates the exact export bytes and the reviewed trust profile. It does not prove to a verifier without database access that the local export receipt exists, and the envelope itself remains plaintext. Signing authorization, private-key lifecycle and signing-system audit remain deployment-owned. The separate [encryption command](audit-export-encryption.md) can protect that exact envelope for an identity-proven X25519 recipient, and the [delivery lifecycle](audit-export-delivery.md) can bind the encrypted package to a deployment-owned destination and the same active [recipient trust generation](audit-export-recipient-trust.md), verify it at one immutable S3 object under an active signed [transport workload identity](audit-export-workload-identity.md), and require a [recipient-signed receipt](audit-export-delivery-receipts.md). The [revocation importer](audit-export-revocation-acquisition.md) can acquire either signed notice type through a registered source, but workload-identity and recipient-proof issuance, monitoring completeness, acquisition-credential issuance and remote revocation, production transport certification, access policy, retention, legal hold, and deletion remain separate boundaries.
