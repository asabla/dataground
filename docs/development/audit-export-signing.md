...
The signature authenticates the exact export bytes and the reviewed trust profile. It does not prove to a verifier without database access that the local export receipt exists, and the envelope itself remains plaintext. Signing authorization, private-key lifecycle and signing-system audit remain deployment-owned. The separate [encryption command](audit-export-encryption.md) can protect that exact envelope for an identity-proven X25519 recipient, and the [delivery acknowledgement command](audit-export-delivery.md) can bind the encrypted package to a deployment-owned destination and the same active [recipient trust generation](audit-export-recipient-trust.md) before requiring a [recipient-signed receipt](audit-export-delivery-receipts.md). Upload and transport execution, proofing- and revocation-authority governance, external notice acquisition, access policy, retention, legal hold, and deletion remain separate boundaries.
