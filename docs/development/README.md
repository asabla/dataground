...
PostgreSQL 18 is required for durable integration tests and durable control-plane operation. The ordinary local verification command skips database integration tests when `DATAGROUND_TEST_DATABASE_URL` is absent; CI requires them. Use a dedicated disposable PostgreSQL server and a test role that can create and drop databases. The worker startup test creates one uniquely named database and removes it on completion so it does not share schema resets with the persistence suite. The OpenShell CLI is required for the opt-in local execution profile described in [local OpenShell guidance](openshell-local.md). Docker also runs the isolated live topology and enforcement checks in CI; neither dependency is required by the ordinary reference runtime. Kubernetes and Rosetta are not reference-runtime dependencies. CI separately builds the pinned Rosetta candidate for the loopback HTTP adapter conformance described in [OpenShell development](openshell-local.md).
...
After an exact service revision exists, an authorized operator can install its reviewed Cedar invocation policy with `go run ./cmd/dataground-policy-install` and the required `-isolation-domain`, `-service`, `-revision`, `-policy-set`, `-policy-file`, `-entity-file`, `-actor`, `-reason`, and `-correlation-id` flags. The command requires the current PostgreSQL schema, accepts one bounded regular policy file and one bounded owner-only direct regular file containing byte-exact canonical Cedar entity JSON, validates the complete bundle against the entity-aware invocation schema, and atomically records immutable policy and entity bytes plus safe installation audit evidence. Its database credential is the administrative authorization boundary. Identical replay is read-only; changed policy, entities, or attribution for the same revision is rejected. The internal `dataground-policy-entity-refresh` command can then `publish` the next complete canonical snapshot generation without activating it and independently `activate` that exact generation against the installed bundle's lowercase `sha256:` digest. Both operations require the exact scope, next generation, actor, reason, and correlation; publication additionally requires the owner-only `-entity-file`, while activation requires `-policy-digest`. The durable source resolves only the latest sequential activation, recomputes its effective bundle digest, and fails closed on gaps, drift, withdrawal, or incomplete evidence. A narrowly held database-authorized operator can irreversibly withdraw the installed bundle with `go run ./cmd/dataground-policy-withdraw`; withdrawal serializes with refresh and blocks all subsequent resolution or activation while preserving historical records. These are internal operator boundaries, not public policy administration, external distribution, identity synchronization, running-execution discovery or termination, or live-sandbox mutation.
...
The [operator-owned gateway command](development-gateway.md) prepares the checked local topology and retains it across worker replacement, with exact startup replay and explicit stop. The [development provider command](development-provider.md) configures the fixed Codex provider with an owner-only consumed credential bundle and exact creation replay. The [development enforcement command](development-enforcement.md) compiles the checked strict input through Rosetta and installs its immutable policy object and audited catalog binding. The [development publication command](development-publication.md) verifies the strict local inputs and atomically publishes the prepared revision. The [queued publication commands](governed-publication-queue.md) accept immutable reviewed inputs and consume one exact operation with lease-fenced verification and restart recovery. An explicit API configuration accepts authenticated publication for one reviewed draft through a version 4 operation and the [publication authorizer](publication-authorization.md), with append-only entry/effect decisions before atomic completion. The worker remains deterministic by default. [Governed development worker guidance](governed-worker.md) documents the explicit single-domain mode that composes exact policy resolution and audit with OpenShell admission, Codex runtime execution, cancellation, and artifact finalization without claiming production certification. That mode also requires a current sequential provider-profile grant for the exact isolation domain and service revision. It evaluates and append-only audits the secret-free profile selection before provider placement and immediately before sandbox creation; missing, expired, revoked, substituted, or cross-domain grants fail closed. OpenShell retains the actual provider credential and routing configuration.
