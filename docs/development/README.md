...
After an exact service revision exists, an authorized operator can install its reviewed Cedar invocation policy with `go run ./cmd/dataground-policy-install` and the required `-isolation-domain`, `-service`, `-revision`, `-policy-set`, `-policy-file`, `-entity-file`, `-actor`, `-reason`, and `-correlation-id` flags. The command requires the current PostgreSQL schema, accepts one bounded regular policy file and one bounded owner-only direct regular file containing byte-exact canonical Cedar entity JSON, validates the complete bundle against the entity-aware invocation schema, and atomically records immutable policy and entity bytes plus safe installation audit evidence. Its database credential is the administrative authorization boundary. Identical replay is read-only; changed policy, entities, or attribution for the same revision is rejected. A narrowly held database-authorized operator can irreversibly withdraw that exact bundle with `go run ./cmd/dataground-policy-withdraw` and the required `-isolation-domain`, `-service`, `-revision`, `-policy-digest`, `-actor`, `-reason`, and `-correlation-id` flags. The supplied lowercase `sha256:` digest must match the installed bundle, the append-only withdrawal and minimized audit evidence commit atomically, and all subsequent durable policy resolution for the revision fails closed while historical bundle reads remain available. This is an internal operator boundary, not public policy authoring, approval workflow, signing, distribution, dynamic entity refresh, running-execution discovery or termination, or reload.
