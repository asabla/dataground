...
The [operator-owned gateway command](development-gateway.md) prepares the checked local topology and retains it across worker replacement, with exact startup replay and explicit stop. The [development provider command](development-provider.md) configures the fixed Codex provider with an owner-only consumed credential bundle and exact creation replay. The [development enforcement command](development-enforcement.md) compiles the checked strict input through Rosetta and installs its immutable policy object and audited catalog binding. The [development publication command](development-publication.md) verifies the strict local inputs and atomically publishes the prepared revision. The internal [queued publication boundary](governed-publication-queue.md) adds immutable reviewed inputs and lease-fenced verification; it is not yet selected by the public API or worker command. The worker remains deterministic by default. [Governed development worker guidance](governed-worker.md) documents the explicit single-domain mode that composes exact policy resolution and audit with OpenShell admission, Codex runtime execution, cancellation, and artifact finalization without claiming production certification. That mode also requires a current sequential provider-profile grant for the exact isolation domain and service revision. It evaluates and append-only audits the secret-free profile selection before provider placement and immediately before sandbox creation; missing, expired, revoked, substituted, or cross-domain grants fail closed. OpenShell retains the actual provider credential and routing configuration.
