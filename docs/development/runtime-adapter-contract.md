...
The common cases require ordered text, tool, process, and file activity; repeated and corrected cumulative usage snapshots; success and failure completion; explicit interruption; malformed protocol rejection; scope mismatch rejection; safe process failure; single-use session ownership; and invalid start rejection before native admission. An adapter must reject a second invocation even after its first turn has completed. Cancelling an observer's wait must not cancel the turn. Repeated waits preserve the terminal result, and repeated close calls are safe. A successful turn returns a nil wait result; an interrupted turn returns `runtime.ErrTurnInterrupted` while retaining its `lifecycle.cancelled` event. Cancelling a wait context is an observer outcome and does not prove native interruption. The suite permits either a closed event channel or an open idle channel after completion. Fixture events must be valid JSON, fit within the kit's 128 KiB envelope ceiling, and exclude the private marker. That fixture ceiling does not replace the production limits on each payload type.
...
The usage scenario requires three `usage.recorded` snapshots with `(inputTokens, outputTokens, totalTokens)` values `(12, 8, 20)`, `(12, 8, 20)`, and `(10, 6, 16)`, then successful completion. Fixtures must provide native cumulative totals and distinct last-request counts where the protocol supports both. The adapter must preserve repeated totals and corrections without adding them. See [runtime usage reporting](runtime-usage.md) for persistence and evidence limits.
