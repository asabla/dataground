...
The [local acceptance command](local-runtime-acceptance.md) can bind a diagnostic and its independently verified published image to an explicit, signed development target. It remains a separate contract from CI certification and does not activate the worker.
