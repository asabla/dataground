...
`go run ./cmd/dataground-worker publish-development` publishes one existing draft Codex revision after checking its reviewed inputs. This is an internal, database-authorized operator command for the strict Linux ARM64 local acceptance profile. For durable queue acceptance and recovery across worker replacement, use the separate [queued publication commands](governed-publication-queue.md). The separate [authorized publication profile](governed-publication-queue.md#publish-through-the-authenticated-api) composes authenticated public acceptance with a version 5 policy and version 4 publication operation. Production publication, deployment readiness, traffic rollout, and release acceptance remain separate work.
