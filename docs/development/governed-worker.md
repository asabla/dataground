...
The governed process also owns question expiry for its configured isolation domain. Before startup completes it commits one bounded expiry batch, then polls independently of the potentially blocking runtime reconciler every 250 ms. Each batch has a two-second database deadline and selects at most 100 due questions using PostgreSQL time and row-lock skipping. Expiry atomically records the terminal state, audit and outbox evidence under actor `dataground-question-expiry` and a fresh batch correlation; delivery already reserved becomes `delivery_unknown` and is never retried. Lost worker leases do not prevent expiry, and concurrent governed processes safely divide due rows through the database locks.

An expiry failure, a batch that exceeds its deadline, stale progress, or owner shutdown withholds governed readiness at the existing execution boundaries. A later successful batch restores that part of readiness without bypassing runtime certification. Expiry continues while certification readiness is lost, and shutdown cancels and joins the owner before closing other worker resources or the database pool. Reference mode starts no expiry owner. This supplies lifecycle maintenance for existing question records. The internal driver also has the question mediation described in [OpenShell development](openshell-local.md), but this command still builds question-disabled requests; question-mode activation and live question certification remain separate work. A running governed process is still required for this development maintenance path.
