...
7. **Local OpenShell topology:** OpenShell 0.0.86, the Docker driver, loopback gateway, immutable images, deny-all fixture, and Codex app-server stdio are pinned under `deploy/openshell`. The internal provider has PostgreSQL-backed, isolation-scoped gateway placement and restart recovery. The initial Codex protocol adapter covers handshake, one ephemeral thread/turn, bounded normalized events, interruption, and fail-closed approvals; richer capabilities, public admission, live service routing, and credential non-exposure certification remain blocked.
