...
function verifyEvidence(evidence) {
	...
	if (
		...
		failures.push(
			"evidence does not match the checked-in OpenShell runtime profile",
		);
	}
	if (
		...
		JSON.stringify(contract?.resourceNames) !==
			JSON.stringify(expectedResourceNames) ||
		...
		failures.push(
			"evidence does not use the checked-in runtime verifier contract",
		);
	}
	...
	if (
		...
		failures.push(
			"evidence must contain every checked runtime case exactly once",
		);
	}
	const commitments = evidence.checks.map(
		(check) => check.observationCommitment,
	);
	if (commitments.length !== new Set(commitments).size) {
		failures.push(
			"each runtime check must have a distinct observation commitment",
		);
	}
	...
	if (
		...
		Object.entries(expectedClassifications).some(
			([name, classification]) =>
				capabilities.get(name)?.classification !== classification,
		) ||
		Object.entries(contract?.capabilityClassifications ?? {}).some(
			([name, classification]) =>
				expectedClassifications[name] !== classification,
		) ||
		Object.keys(contract?.capabilityClassifications ?? {}).length !==
			Object.keys(expectedClassifications).length
	) {
		failures.push(
			"runtime capability classifications drift from the checked profile",
		);
	}
	...
	if (
		...
		failures.push(
			"runtime capability evidence or limitation classification is inconsistent",
		);
	}
	...
	for (const check of evidence.checks) {
		...
		if (
			...
			failures.push(
				`${check.name} observation window is outside the evidence run`,
			);
		}
	}
	...
	if (
		Object.entries(names).some(
			([kind, expected]) => evidence.run.resources[kind] !== expected,
		) ||
		...
		failures.push(
			"runtime resources and cleanup are not bound to the evidence run",
		);
	}
	return failures;
}
...
function representativeEvidence() {
	...
	return {
		...
		checks,
		capabilities: Object.entries(expectedClassifications).map(
			([name, classification]) => ({
				name,
				classification,
				evidence: structuredClone(expectedEvidence[name] ?? []),
				reasonCode:
					classification === "supported" ? null : "ADAPTER_UNSUPPORTED",
			}),
		),
		cleanup: {
	...
	const mutations = [
		...
		[
			"duplicate capability",
			{
				...
				capabilities: valid.capabilities.map((capability, index) =>
					index === 1
						? { ...capability, name: valid.capabilities[0].name }
						: capability,
				),
			},
			false,
		...
		[
			"stale observation window",
			{
				...
				checks: valid.checks.map((check, index) =>
					index === 0
						? { ...check, startedAt: "2026-07-30T11:59:59.000Z" }
						: check,
				),
			},
			false,
