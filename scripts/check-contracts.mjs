...
const validateResourceAuditPage = ajv.compile({
  $ref: "urn:dataground:openapi:v1#/components/schemas/ResourceAuditPage",
});
const auditPage = await readJson("contracts/fixtures/valid/resource-audit-page.json");
assert.equal(validateResourceAuditPage(auditPage), true);
assert.equal(validateResourceAuditPage({ ...auditPage, items: [] }), true);
assert.equal(
  validateResourceAuditPage({ ...auditPage, items: Array(100).fill(auditPage.items[0]) }),
  true,
);
assert.equal(
  validateResourceAuditPage({ ...auditPage, items: Array(101).fill(auditPage.items[0]) }),
  false,
);
for (const field of ["safeMetadata", "sequence", "payload", "snapshot"]) {
  assert.equal(
    validateResourceAuditPage({
      ...auditPage,
      items: [{ ...auditPage.items[0], [field]: "private" }],
    }),
    false,
  );
}
assert.equal(validateResourceAuditPage({ ...auditPage, nextCursor: "1:1234" }), false);
assert.equal(
  validateResourceAuditPage({ ...auditPage, schemaVersion: "dataground.resource-audit-page/v2" }),
  false,
);
...
for (const fixture of fixtureManifest.fixtures) {
  ...
  if (
    ...
    if (fixture.document === "audit-export-delivery-receipt-v4") {
      const destinationDigest = createHash("sha256")
        .update(`${JSON.stringify(validDeliveryDestinationFixture)}\n`)
        .digest("hex");
      const objectDigest = validDeliveryDestinationFixture.objectKey
        .split("/")
        .at(-1)
        .replace(/\.json$/, "");
      assert.equal(
        value.content.destinationSha256,
        `sha256:${destinationDigest}`,
        `${fixture.file} must bind the destination fixture`,
      );
      assert.equal(
        value.content.encryptedPackageSha256,
        `sha256:${objectDigest}`,
        `${fixture.file} and destination must bind the same encrypted package`,
      );
      assert.equal(value.content.deliveryId, validDeliveryDestinationFixture.deliveryId);
      assert.equal(
        value.content.isolationDomainId,
        validDeliveryDestinationFixture.isolationDomainId,
      );
      assert.equal(value.content.recipientId, validDeliveryDestinationFixture.recipientId);
    }
  }
