...
type auditExportProofingAuthorityRepository interface {
	ChangeAuditExportProofingAuthority(
		context.Context,
...
func main() {
	if err := run(context.Background(), os.Args[1:]); err != nil {
		fmt.Fprintln(os.Stderr, "DataGround audit export proofing authority failed")
		os.Exit(1)
	}
}
...
func run(ctx context.Context, arguments []string) error {
	...
	if ctx == nil {
		return errors.New("audit export proofing authority context is required")
	}
	...
	if !change.Valid() {
		return errors.New("audit export proofing authority change is invalid")
	}
...
func authorityEvidence(request commandRequest) (auditseal.ProofingAuthorityTrustEvidence, error) {
	if request.operation == "activate" {
		evidence, err := auditseal.InspectProofingAuthorityTrustFile(request.trustFile)
		if err != nil {
			return auditseal.ProofingAuthorityTrustEvidence{}, err
		}
		if evidence.AuthorityID != request.authorityID {
			return auditseal.ProofingAuthorityTrustEvidence{},
				errors.New("audit export proofing authority profile does not match")
		}
		return evidence, nil
	}
	return auditseal.ProofingAuthorityTrustEvidence{
		Contract: auditseal.RecipientProofingTrustContract,
		SHA256:   request.trustSHA256, AuthorityID: request.authorityID,
	}, nil
}

func executeRequest(
	ctx context.Context,
	repository auditExportProofingAuthorityRepository,
	change persistence.AuditExportProofingAuthorityChange,
) error {
	if repository == nil {
		return errors.New("audit export proofing authority repository is required")
	}
	return repository.ChangeAuditExportProofingAuthority(ctx, change)
}
...
func newAuthorityChange(
	...
	return persistence.AuditExportProofingAuthorityChange{
		...
		AuthorityID: evidence.AuthorityID, Generation: request.generation,
		TrustContract: evidence.Contract, TrustProfileSHA256: evidence.SHA256,
...
func parseArguments(arguments []string) (commandRequest, error) {
	...
	flags := flag.NewFlagSet("dataground-audit-export-proofing-authority", flag.ContinueOnError)
	...
	flags.StringVar(&request.authorityID, "authority", "", "deployment-owned proofing authority identifier")
	flags.Int64Var(&request.generation, "generation", 0, "sequential authority generation")
	flags.StringVar(&request.trustFile, "trust-file", "", "canonical proofing authority trust profile")
	...
	if err := flags.Parse(arguments); err != nil {
		return commandRequest{}, errors.New("audit export proofing authority arguments are invalid")
	}
	if flags.NArg() != 0 || request.isolationDomainID == "" || request.authorityID == "" ||
		request.generation < 1 || request.actorID == "" || request.reason == "" ||
		request.correlationID == "" {
		return commandRequest{}, errors.New("audit export proofing authority arguments are incomplete")
	}
	if request.operation != "activate" && request.operation != "revoke" {
		return commandRequest{}, errors.New("audit export proofing authority operation is invalid")
	}
	if (request.operation == "activate" && (request.trustFile == "" || request.trustSHA256 != "")) ||
		...
		return commandRequest{}, errors.New("audit export proofing authority evidence is invalid")
	}
	if !validReason(request.reason) {
		return commandRequest{}, errors.New("audit export proofing authority reason is invalid")
	}
	return request, nil
}
