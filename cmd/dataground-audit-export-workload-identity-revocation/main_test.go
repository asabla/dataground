...
func TestParseArgumentsRequiresClosedRevocationIntake(t *testing.T) {
	base := []string{
		...
		"-reason", "record external workload identity key revocation",
...
func TestExecuteRequestForwardsExactVerifiedRevocation(t *testing.T) {
	...
	verified := auditseal.VerifiedWorkloadIdentityRevocation{
		...
		WorkloadIdentitySigningKeyID:       "workload_issuer_key_01",
	...
	repository := &workloadIdentityRevocationRepositoryStub{}
...
type workloadIdentityRevocationRepositoryStub struct {
	calls  int
	record persistence.AuditExportWorkloadIdentityRevocationRecord
}

func (repository *workloadIdentityRevocationRepositoryStub) RecordAuditExportWorkloadIdentityRevocation(
	_ context.Context,
	record persistence.AuditExportWorkloadIdentityRevocationRecord,
) error {
	if !record.Valid() {
