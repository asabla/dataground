...
func run(ctx context.Context, arguments []string, output io.Writer) error {
	flags := flag.NewFlagSet("dataground-operator-audit-export", flag.ContinueOnError)
	flags.SetOutput(io.Discard)
	var exportID, domainID, requestedBy, reason, correlationID, cursor string
	var limit int
	flags.StringVar(&exportID, "export-id", "", "stable operator audit export identifier")
	...
	if flags.NArg() != 0 || exportID == "" || domainID == "" || requestedBy == "" || reason == "" ||
		correlationID == "" || limit < 1 || limit > 1000 {
	...
	document, err := persistence.NewRepository(pool).ExportOperatorAuditRecordsAudited(
		operationCtx,
		persistence.OperatorAuditExportRequest{
			ExportID:          exportID,
	...
	if _, err := fmt.Fprintln(output, string(document)); err != nil {
		return fmt.Errorf("write operator audit export: %w", err)
	}
	return nil
}
