...
func TestPinnedOIDCJWTVerifierRejectsInvalidSignatureHeaderAndClaims(t *testing.T) {
	...
	invalidConfirmation := fixture.signPayload(t, []byte(fmt.Sprintf(
		`{"iss":%q,"sub":%q,"aud":%q,"exp":%d,"iat":%d,"cnf":{"jkt":"not-a-thumbprint"}}`,
		testOIDCIssuer,
		testOIDCSubject,
		testOIDCAudience,
		now.Add(5*time.Minute).Unix(),
	...
	tests := map[string]string{
		...
		"invalid confirmation": invalidConfirmation,
