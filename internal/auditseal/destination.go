...
func VerifyDeliveryDestinationFile(
	...
	if !validDeliveryDestinationTransport(destination) {
...
func validDeliveryDestinationTransport(destination DeliveryDestination) bool {
	switch destination.Contract {
	case DeliveryDestinationContract:
		return destination.TransportContract == persistence.AuditExportDeliveryTransportContract &&
			destination.ClientCertificateSHA256 == "" && destination.ServerTrustSHA256 == ""
	case DeliveryDestinationMTLSContract:
		return destination.TransportContract == persistence.AuditExportDeliveryMTLSTransportContract &&
