...
The deterministic reference runtime supports loopback-only process-local operation and PostgreSQL-backed durable development. OpenShell, enforcement, admission, native runtime, artifact, recovery, and credential-evidence components remain internal, opt-in, development-scoped, or unwired unless the canonical documentation and verified tests explicitly say otherwise. A strict injected HTTP authentication boundary and loopback-only static development verifier now bind API requests, actor attribution, isolation-domain membership, and idempotency replay to one validated principal. Production OIDC verification, Cedar action authorization, provider credential mediation, default or public execution composition, production backend selection, production artifact storage, and production infrastructure remain unimplemented. Cedar authorization is available only through explicit immutable policy bundles and opt-in static composition; no default policy or worker wiring is configured. Do not claim live-runtime or production certification beyond evidence incorporated in the canonical documentation.
