...
The deterministic reference runtime has a loopback-only process-local mode and a PostgreSQL-backed durable development mode. Durable mode persists commands, explicit publication and invocation operations, event replay, idempotency, outbox and audit records, and fenced external-effect receipts through replaceable workers. Neither mode implements authentication, Cedar authorization, OpenShell execution, provider credential mediation, or production artifact storage; `docs/development/reference-runtime.md` and `docs/development/durable-control-plane.md` record the supported behavior and limitations. Complete the governed agent-service vertical slice before integrating broad notebook, lakehouse, job, or compatibility surfaces.
